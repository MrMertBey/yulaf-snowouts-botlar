...
module.exports.execute = async (client, message, args) => {
    ...
    if(!reason) return message.reply("Geçerli bir sebep belirtmelisin");
    ...
    message.channel.send(`${victim}, ${message.author} tarafından **${reason}** sebebiyle geçici olarak seslide susturuldu, Ceza Numarası - (\`#${document.Id}\`) ${Settings.emojiler.tik}`)
    message.react(Settings.emojiler.onayID)
    message.guild.log(message.author, victim, document, Settings.Penals.VoiceMute.Log);
}

module.exports.settings = {
    Commands: ["voicemute", "seslisustur","vmute"],
