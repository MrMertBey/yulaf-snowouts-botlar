...
module.exports = {
  conf: {
    aliases: ["yetki"],
    name: "rank",
    help: "rank [ekle/sil] [coin] [rol] / [temizle] / [liste]",
  ...
  run: async (client, message, args, embed) => {
    ...
    if (["ekle", "add"].includes(args[0])) {
      ...
      message.channel.send(embed.setColor('GRAY').setDescription(`${coin} coine ulaşıldığında verilecek roller ayarlandı! \nVerilecek Roller: ${roles.map((x) => `<@&${x.id}>`).join(", ")}`));
      ...
      message.channel.send(embed.setColor('GRAY').setDescription(`${coin} coine ulaşıldığında verilecek roller silindi!`));
    } else if (["temizle", "clear"].includes(args[0])) {
      global.rankdb.set("ranks", []);
      message.channel.send(embed.setColor('GRAY').setDescription("Tüm yetkiler başarıyla temizlendi!"));
    } else if (["liste", "list"].includes(args[0])) message.channel.send(embed.setColor('GRAY').setDescription(global.rankdb.get("ranks").sort((a, b) => a.coin - b.coin).map((x) => `${Array.isArray(x.role) ? x.role.map(x => `<@&${x}>`).join(", ") : `<@&${x.role}>`}: ${x.coin}`)));
  }
};
