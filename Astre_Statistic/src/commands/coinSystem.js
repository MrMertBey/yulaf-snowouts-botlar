module.exports = {
  ...
  run: async (client, message, args, embed) => {
    if (["aç", "open"].includes(args[0])) {
      ...
      message.channel.send(embed.setColor("GRAY").setDescription("Coin sistemi başarıyla açıldı!"));
      ...
      message.channel.send(embed.setColor("GRAY").setDescription("Coin sistemi başarıyla kapatıldı!"));
    } else message.channel.send(embed.setColor("GRAY").setDescription(`Coin sistemi: \`${global.confdb.get("coinSystem") ? "açık" : "kapalı"}\``));
  }
};