...
module.exports = {
  conf: {
    aliases: ["görev"],
    name: "task",
    help: "görev [ver/sil] [kullanıcı] [invite/ses/mesaj/taglı/kayıt] [süre] [miktar(ses)] [ödül miktarı] [kanallar(tercih)] / [sil] [görev ID] / al [görev tipi]? / şema [ekle/sil/liste/bilgi]",
    ...
    if (["ver", "give"].includes(args[0])) {
      ...
      if (["şema", "schema"].includes(args[2])) {
        ...
        if (role) {
          ...
          message.channel.send(embed.setColor("GRAY").setDescription(`${message.mentions.roles.first().toString()} rolüne sahip olan tüm üyelere başarıyla ${id} ID'li şemadan görevi verildi! \nGörev verilen üyeler: ${members.map((x) => x.toString()).join(", ")}`));
        } else {
          ...
          message.channel.send(embed.setColor("GRAY").setDescription(`${member.toString()} üyesine başarıyla ${id} ID'li şemadan verildi!`));
        }
      }
      ...
      if (role) {
        ...
        message.channel.send(embed.setColor("GRAY").setDescription(`${message.mentions.roles.first().toString()} rolüne sahip olan tüm üyelere başarıyla ${type} görevi verildi! \nGörev verilen üyeler: ${members.map((x) => x.toString()).join(", ")}`));
      } else {
        ...
        message.channel.send(embed.setColor("GRAY").setDescription(`${member.toString()} üyesine başarıyla ${type} görevi verildi!`));
      }
    } else if (["sil", "delete", "al"].includes(args[0])) {
      if (!message.member.hasPermission(8)) return message.channel.error(message, "Yeterli yetkin bulunmuyor!");
      const id = args[1];
      ...
      message.channel.send(embed.setColor("GRAY").setDescription(`${id} ID'li görev başarıyla silindi!`));
      ...
      message.channel.send(embed.setColor("GRAY").setDescription(`
      ...
      if (["ekle", "add"].includes(args[1])) {
        ...
        message.channel.send(embed.setColor("GRAY").setDescription(`${id} ID'li görev şeması başarıyla silindi!`));
      } else if (["liste", "list"].includes(args[1])) message.channel.sendEmbed(embed.setColor("GRAY").setDescription(client.tasks.map((x) => `\`#${x.id}\` ${x.type} ${x.count} ${x.prizeCount} ${x.channels ? x.channels.map((x) => `<#${x}>`) : ""}`).join("\n")));
